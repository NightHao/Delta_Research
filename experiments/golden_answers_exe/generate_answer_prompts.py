...
def create_prompt_for_question(question_data):
    ...
    all_info = question_data.get('gathered_information', [])
...
def generate_prompts(data, output_dir):
    ...
    # Check if data is already a list (from gathered_information_with_images.json)
    # or if it's a dictionary with a 'questions' key (from gathered_information.json)
    ...
    for question_data in questions:
        ...
        filename = f"{safe_question[:50]}.txt"
